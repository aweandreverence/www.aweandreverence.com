...
- URLs default to a slug derived from the title plus the post ID. When retitling a published post, set optional `slug` front matter to its existing title-derived slug (without the post ID) to preserve shared links. For example, `slug: 'a-righteous-man-falls-seven-times-and-rises-again'` keeps that article at `/blog/a-righteous-man-falls-seven-times-and-rises-again-f7a2416/` even when its title changes.
- Write Scripture references as plain text (for example, `Romans 3:21–26`), including the full range. The site already loads `manna.js` in `src/pages/_document.js` to add Bible links and passage previews; manually linked references are skipped. Keep any quoted Scripture and its required copyright notice in the article.
